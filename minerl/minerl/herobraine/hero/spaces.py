...
class Box(gym.spaces.Box, MineRLSpace):
    ...
    def sample(self, bs=None):
        ...
        * (-oo, oo) : normal distribution
        ...
        sample[..., unbounded] = self.np_random.normal(size=bdim + unbounded[unbounded].shape)

        sample[..., low_bounded] = (
            self.np_random.exponential(size=bdim + low_bounded[low_bounded].shape) + self.low[low_bounded]
        )

        sample[..., upp_bounded] = (
            -self.np_random.exponential(size=bdim + upp_bounded[upp_bounded].shape) - self.high[upp_bounded]
        )

        sample[..., bounded] = self.np_random.uniform(
            low=self.low[bounded], high=high[bounded], size=bdim + bounded[bounded].shape
        )
...
class Dict(gym.spaces.Dict, MineRLSpace):
    ...
    def create_flattened_space(self):
        shape = sum([s.flattened.shape[0] for s in self.spaces.values() if s.is_flattenable()])
    ...
    def flat_map(self, x):
        ...
        try:
            ...
            for k, v in self.spaces.items():
                if v.is_flattenable():
                    stuff_to_cat.append((v.flat_map(x[k]) if k in x else v.flat_map(v.no_op(batch_shape))))
    ...
    def unflattenable_map(self, x: OrderedDict) -> OrderedDict:
        ...
        return OrderedDict(
            {
                k: (v.unflattenable_map(x[k]) if hasattr(v, "unflattenable_map") else x[k])
                # filter
                for k, v in (self.spaces.items())
                if not v.is_flattenable()
            }
...
class Text(MineRLSpace):
    ...
    def sample(self,batch_size=None):
        ...
        strings = [
            ...
            for _ in range(int(total_strings))
        ]
        return np.array(np.reshape(strings, self.shape))
